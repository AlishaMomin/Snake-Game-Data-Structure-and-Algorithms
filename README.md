...
Class square:
This function is responsible for the change in the direction of snake
a directory with a tuple of pos indicate change in direction in case of striking a direction key i.e. (1,0)(0,1)(-1,0)(0,-1)
Upon each strike the program would draw a rectangle in the similar direction.
To keep the snake moving:
Pos of snake keeps getting updated by adding the dir_snake which basically is the next immediate state of snake in terms of dir, it grow literally by a unit of one. In case of change in dir dir_snake would change respectively according to the above stated program.
Collision:
...
step1: give the color code to apple and head of the snake 
step2: for every i in self.squares [1:] it will draw the apple and snake 
Def set_direction (self, direction):
Basically, this function will check the directions and will return the coordinates according to the given position. 
step1: it will check the position 
step2: it will return the direction at a given coordinates  

Def handle_events (self):
This function handles 2 events that is quit game and keyboard 
step1: if the type of the event is QUIT then the pygame will be end and the display will be closed 
step2: else it will take the keyboard input. If the keyboard input is equal to the given position then it will return the position which will help snake to move according to the position direction.
...
step1: this function will check if the snake head hits any part of its body then game will be end.
step2: else the game is continued to be played. 
...
step1: we will be generating apples in the range of 17 which is our row and column. 
step2: it will check if the first apple is eaten by snake then it will create the next apple by following the step1 and so on 
...
step1: it will check if the snake head is equal to apple then it will generate a new apple by calling the above generate_apple function
step2: once it has generated a new apple then it will increase the score by 1.
Def go_to (self, position):
This function is basically set the direction of head to the position of target.
We have used BFS algorithm which we have done in labs. Lastly we have few more functions which is given below 
...
step1: we have initialized the pygame by using pygame.init and then caption our display screen which is DSA Final project Snake Game
step2: the game is running at the speed of 50 as we have used the pygame.time.Clock ().tick function
step3: first it will check if the gameLoop is true (which means if the snake has not collided with its body or wall then gameLoop==true) then it will call the above classes and function to run the game. And every time it will update the screen after we have called the above class. This will show the new movement of snake. 
step3: if the gameLoop==false then will stop the game and our pygame display will be closed.
